...
class FrozenOpenCLIPEmbedder(torch.nn.Module):
    ...
    def encode_with_transformer(self, text):
        ...
        # insert learned embeddings <start>
        current_run_path = os.getcwd()
        print(current_run_path)
        learned_embeddings = torch.load(os.path.join(current_run_path, "modelscope_t2v_files/tmp.pt")).cuda()
        v1_emb = learned_embeddings[:, 0]
        v2_emb = learned_embeddings[:, 1]

        for index in range(len(text[0])):
            if text[0][index] == 2533 and text[0][index + 1] == 2533:
                x[0][index] = v1_emb
                x[0][index + 1] = v2_emb
                break
            if text[0][index] == 49407:
                break
        # insert learned embeddings <end>
