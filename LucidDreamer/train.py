...
def guidance_setup(guidance_opt):
    if guidance_opt.guidance=="SD":
        ...
        # StableIdentity
        print("Inserting StableIdentity into LucidDreamer!")
        ...
        guidance.pipe.tokenizer.add_tokens(tokens)
        token_ids = guidance.pipe.tokenizer.convert_tokens_to_ids(tokens)

        # resize token embeddings and set new embeddings
        guidance.pipe.text_encoder.resize_token_embeddings(len(guidance.pipe.tokenizer), pad_to_multiple_of = 8)
        for token_id, embedding in zip(token_ids, embeddings):
            guidance.pipe.text_encoder.get_input_embeddings().weight.data[token_id] = embedding        
...
def prepare_output_and_logger(args):    
    if not args._model_path:
        ...
        args._model_path = os.path.join("./LucidDreamer/output/", args.workspace)
